...
```{r}
msleep_clean <- mutate(msleep_clean, sleep_total = if_else(sleep_total > 18, NA, sleep_total))
```
...
It produces the scatterplot we asked for, but you'll also see a warning saying "Removed 27 rows containing missing values (geom_point)" to remind you that it's ignoring any observations that are missing for either of the two variables we're plotting.
